        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card">
              <div class="flex items-start gap-6 mb-6">
                <div class="w-24 h-24 rounded-2xl bg-gradient-to-br from-accent to-accent-light flex items-center justify-center text-background font-bold text-3xl">
          ...
          <div class="space-y-6">
            ...
            <div class="card">
              ...
              <div class="flex items-center justify-center mb-6">
                <div class="relative w-32 h-32">
                  <svg class="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                    <circle cx="50" cy="50" r="45" fill="none" stroke="currentColor" class="text-surface-light" stroke-width="8"/>
                    <circle cx="50" cy="50" r="45" fill="none" [attr.stroke]="getRiskColor(patient().riskScore)" stroke-width="8" stroke-linecap="round" [attr.stroke-dasharray]="getStrokeDasharray(patient().riskScore)" stroke-dashoffset="0"/>
                  </svg>
              ...
              <div class="text-center">
                @if ((patient().riskScore || 0) < 30) {
                  <span class="px-4 py-2 rounded-full text-sm font-semibold bg-success/20 text-success">{{ patient().riskCategory || 'Faible' }}</span>
                } @else if ((patient().riskScore || 0) < 70) {
                  <span class="px-4 py-2 rounded-full text-sm font-semibold bg-warning/20 text-warning">{{ patient().riskCategory || 'Modéré' }}</span>
                } @else {
                  <span class="px-4 py-2 rounded-full text-sm font-semibold bg-error/20 text-error">{{ patient().riskCategory || 'Élevé' }}</span>
                }
              </div>
            </div>
            ...
            <div class="card">
              ...
              @if (patient().assignedTherapeuteName) {
                <div class="flex items-center gap-4">
                  <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-primary to-primary-light flex items-center justify-center text-white font-bold">
